...
A new translation is created by creating a new translated form of the English-language (or another convenient language, e.g. Spanish, to create Portuguese etc) {openehr_terminology_resources}/tree/master/computable/XML[terminology file^].

This may be done by the Git workflow of:

* fork the {openehr_terminology_resources}[repository^];
* create the new file in a directory `/computable/XML/xx` where `xx` is a ISO 639 2-character language code;
